...
if __name__ == '__main__':
    ...
    for f in functions:
        ...
        im = re_interaction.MEE(f, 20, 100, 0, 0.001, 0.000001, use_mic_value=True)
        IM = im.get_IM()
        print("finished IM")
        meet = re_factors.FactorArchitecture(dim=20)
        meet.MEET(IM)
        print("finished MEET")
        meet.save_architecture("MeetRandom/meet")

        print("Starting Random 20")
        im = re_interaction.RandomTree(f, 20, 100, 0.001, 0.000001)
        IM = im.run(20)
    ...
    fea = FEA(func17, 10, 10, 10, fa, PSO)
    fea.run()
    print()
