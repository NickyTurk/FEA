...
for i,field in enumerate(fields_to_test):
    ...
    for population in population_sizes:
        for ga_run in ga_runs:
            start = time.time()
            filename = path + '/results/FEAMOO/FEAMOO_' + field_names[i] + '_trial_3_objectives_linear_topo_ga_runs_' + str(ga_run) + '_population_' + str(population) + time.strftime('_%d%m%H%M%S') + '.pickle'
