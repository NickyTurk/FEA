...
from utilities.field.create_prescription_datafile import create_pd_dataframe
...
class YieldPredictor:
    ...
    def calculate_yield(self, prescription=None):
        ...
        if prescription:
            ...
            self.nitrogen_dataframe = new_df.loc[:, self.data_headers] # data to predict on, cannot contain cell_index because it is "new", and the predictor can only handle data it has already seen
            nitrogen_dataframe = new_df.loc[:, self.adjusted_data_headers]
