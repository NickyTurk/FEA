...
for i, field in enumerate(fields_to_test):
    ...
    agg_files = [
        "C:\\Users\\f24n127\\Documents\\Work\\Ag\\Data\\henrys\\wood_10m_yldDat_with_sentinel.csv"
    ]  # henrys\\wood_10m_yldDat_with_sentinel.csv"] #broyles_10m_yldDat_with_sentinel.csv"]
    reduced_agg_files = [
        "C:\\Users\\f24n127\\Documents\\Work\\Ag\\Data\\reduced_broyles_sec35mid_cnn_aggregate.csv"
    ]  # henrys\\reduced_wood_10m_yldDat_with_sentinel_aggregate.csv"] #reduced_broyles_10m_yldDat_with_sentinel_aggregate.csv"]
    df = pd.read_csv(agg_files[i])

    """
    Load CNN model
    """
    cnn = YieldMapPredictor(
        filename="C:\\Users\\f24n127\\Documents\\Work\\Ag\\Data\\henrys\\wood_10m_yldDat_with_sentinel.csv",
        field=field.field_name,
        pred_year=2020,
        training_years=[2016, 2018],
    )
    ...
    path_weights = (
        "C:\\Users\\f24n127\\Documents\\Work\\OFPETool-master\\static\\uploads\\Hyper3DNet-"
        + field_names[i]
        + "--Objective-yld\\Hyper3DNet-"
        + field_names[i]
        + "--Objective-yld"
    )
    ...
    """
    lat_lon = [
        utm.to_latlon(cnn.coords[x, y][0], cnn.coords[x, y][1], 12, northern=True)
        for (x, y) in centers
    ]  # utm.to_latlon(cnn.coords[x, y][0], cnn.coords[x, y][1], 12, northern=True
    # add cell ids to centers dataframe
    centers = create_indexed_dataframe(
        dps=pd.DataFrame(lat_lon), field=field
    )  # epsg_string='epsg:2263'
    ...
    random_global_variables = [
        random.randrange(0, upper_bound) for x in range(len(field.cell_list))
    ]  # random.randrange(0, upper_bound)
    ...
    for j in range(5):
        ...
        elapsed = end - start
        print(
            "CCNSGA with ga runs %d and population %d took %s"
