...
agg_files = ["C:/Users/f24n127/Documents/Work/Ag/Data/aggregated_data/broyles_sec35mid_2016_yl_aggreg_20181112.csv"] # wood_henrys_yl18_aggreg_20181203.csv broyles_sec35mid_2016_yl_aggreg_20181112.csv
reduced_agg_files = ["C:/Users/f24n127/Documents/Work/Ag/Data/aggregated_data/reduced_broyles_sec35mid_agg.csv"] #reduced_henrys_agg.csv  reduced_broyles_sec35mid_agg
...
for i, field in enumerate(fields_to_test):
    ...
    for j in range(5):
        start = time.time()
        filename = path + '/results/prescriptions/RF_optimized/' +alg_name+ '_' + field_names[i] + '_strip_trial_3_objectives_ga_runs_' + str(ga_run) + '_population_' + str(population_size) + time.strftime('_%d%m%H%M%S') + '.pickle'
        ...
        feamoo.run()
        # nsga = NSGA2(population_size=population_size, ea_runs=ga_run, dimensions=len(field.cell_list),
        #              value_range=[0,upper_bound], reference_point=[1, 1, 1])
        # nsga.run()
        end = time.time()
        pickle.dump(feamoo, open(filename, "wb"))
        elapsed = end-start
        print("NSGA with ga runs %d and population %d took %s"%(ga_run, population_size, str(timedelta(seconds=elapsed))))
