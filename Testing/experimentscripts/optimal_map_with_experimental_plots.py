...
agg_files = ["C:/Users/f24n127/Documents/Work/Ag/Data/aggregated_data/wood_henrys_yl18_aggreg_20181203.csv",
             "C:/Users/f24n127/Documents/Work/Ag/Data/aggregated_data/broyles_sec35mid_2016_yl_aggreg_20181112.csv"]
reduced_agg_files = ["C:/Users/f24n127/Documents/Work/Ag/Data/aggregated_data/reduced_henrys_agg.csv",
                     "C:/Users/f24n127/Documents/Work/Ag/Data/aggregated_data/reduced_broyles_sec35mid_agg.csv"]

labels = ['yl18_bu_ac', 'yl_2016']
data_to_use = [['x', 'y', 'n_lbs_ac', 'elev_m', 'slope_deg', 'ndvi15', 'ndvi16', 'ndvi17', 'yl16_nn_bu_ac', 'n16_lbs_ac'],  # Henrys
               ['x', 'y', 'n_lbs_ac', 'elev_m', 'slope_deg', 'ndvi_2012', 'ndvi_2014', 'ndvi_2015', 'yl14_nn_bu_ac',  # Sec35Mid
...
for i, field in enumerate(fields_to_test):
    ...
    random_global_variables = random.choices([20, 30, 40, 50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150],
                                             k=len(field.cell_list))
    ...
    FA.factors = field.create_strip_groups(overlap=overlap_bool)  # group sizes are based on strips on the field
    ...
    """
    # File to save to
    filename = path + '/results/prescriptions/CNN_optimized/' + alg_name + '_' + field_names[
        i] + '_strip_trial_3_objectives_ga_runs_' + str(ga_run) + '_population_' + str(population_size) + time.strftime(
        '_%d%m%H%M%S') + '.pickle'
    ...
    prescription_NR_max.overlay_experimental_plots(experiment_percentage=.25, method="stdev")
    prescription_fert_min.overlay_experimental_plots(experiment_percentage=.25, method="stdev")
    ...
    for i, cell in enumerate(field.cell_list):
        fert_cell = deepcopy(cell)  # needs to be a deepcopy since we are assigning different values to this cell object
    ...
    fileIO.ShapeFiles.write_field_to_shape_file(nr_cells, field, shapefile_schema=schema,
                                                filename=path+"/results/optimal_maps_with_exp_plots/FNSGA_net_return")
    fileIO.ShapeFiles.write_field_to_shape_file(fert_cells, field, shapefile_schema=schema,
