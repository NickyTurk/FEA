...
for fieldfile, field_name in zip(field_files, field_names):
    ...
    for method in methods:
        ...
        for obj in objectives:
            to_find = 'D:/Prescriptions/CNN_predictions/' + '/' + method + "_" + field_name + '_prescription_' + obj + '_objective_runs*.csv'
            results = glob.glob(to_find)[0]
            if not results:
                to_find = 'D:/Prescriptions/CNN_predictions/' + field_name + "_" + method + '_prescription_' + obj + '*.csv'
                results = glob.glob(to_find)[0]
            print(results)
            ...
            for i, cell in enumerate(field.cell_list):
                ...
                if np.array(point_in_cell).size > 0:
                    ...
                    if non_zero.size > 0:
                        ...
                        applicator = applicator + np.mean(point_in_cell[point_in_cell.columns[-3]]) * 0.98
            net_return = yld * 5.30 - (
                        applicator * 1.05) - field.fixed_costs
            print('NR: ', net_return)
