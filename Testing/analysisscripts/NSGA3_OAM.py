...
ks = [0.2]  # [.5, .5, .5, .5, .4, .4, .4, .4, .3, .3, .3, .3, .2, .2, .2, .2]
ls = [0.5]  # [.5, .4, .3, .2, .5, .4, .3, .2, .5, .4, .3, .2, .5, .4, .3, .2]
comparing = []
obj = 10
...
for i, problem in enumerate(problems):
    ...
    for k, l in zip(ks, ls):
        comparing.append(
            ["NSGA3_" + str(part), "k_" + str(k).replace(".", "") + "_l_" + str(l).replace(".", "")]
        )
        reference_point = pickle.load(
            open(
                "E:\\reference_points\\" + problem + "_" + str(obj) + "_reference_point.pickle",
                "rb",
            )
        )
        file_regex = (
            r"NSGA3_" + str(part) + "(.*)" + problem + r"_(.*)" + str(obj) + r"_objectives_(.*)"
        )
        stored_files = MultiFileReader(
            file_regex, dir="C:\\Users\\amy_l\\PycharmProjects\\FEA\\results\\factorarchive\\"
        )
        ...
        for id, experiment in enumerate(experiments):
            ...
            if isinstance(results, Result):
                ...
                overlapping_archive = fa.find_archive_overlap(
            ...
            if len(overlapping_archive) > 2 and calc_stats:
                ...
                overlapping_div = po.calculate_diversity(overlapping_fitness, normalized=True)
            ...
            if len(overlapping_archive) > 0 and create_plot:
                try:
                    max_value = np.max(
                        np.array([x.fitness / reference_point for x in overlapping_archive])
                    )
        ...
        print(
            "nsga3: ",
            np.mean(avg_hv["nsga3"]),
            np.std(avg_hv["nsga3"]),
            np.mean(avg_div["nsga3"]),
