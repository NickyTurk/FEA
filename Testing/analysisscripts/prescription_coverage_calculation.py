...
for i in range(5):
    ...
    for method in methods:
        ...
        if feamoo.nondom_archive:
            nondom_solutions[method] = np.array(
                [np.array(x.fitness) for x in feamoo.nondom_archive]
            )
    ...
    total_front = np.vstack(
        (nondom_solutions["CCNSGA2"], nondom_solutions["FNSGA2"], nondom_solutions["SNSGA2"])
    )
    ...
    nsga_coverage = len(
        [x for x in indeces if ccea_len + fea_len <= x <= ccea_len + fea_len + nsga_len]
    ) / len(indeces)
