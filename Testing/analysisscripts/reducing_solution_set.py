...
for i, problem in enumerate(problems):
    ...
    experiments = [x for x in experiment_filenames if comparing[i][0] in x and comparing[i][1] in x and problem in x]
    ...
    for experiment in experiments:
        ...
        if isinstance(results, MOEA):
            ...
            sfam = FactorArchive(obj, 100, percent_best=ks[i], percent_remaining=ls[i])
            sfam.update_archive(results.nondom_pop)
            sfam_overlap = sfam.find_archive_overlap(nr_archives_overlapping=archive_overlap)
            ese_sol_set = environmental_solution_selection_nsga2(results.nondom_archive.flatten_archive(),
                                                                 sol_size=len(overlapping_archive))
            ess_sol_set = environmental_solution_selection_nsga2(results.nondom_pop,
                                                                 sol_size=len(sfam_overlap))
        ...
        nsga2_fitness = np.array([np.array(sol.fitness) / reference_point for sol in results.nondom_pop])
        ...
        if len(sfam_overlap) > 2:
            sfam_fitness = np.array([np.array(sol.fitness) / reference_point for sol in sfam_overlap])
        ...
        if len(overlapping_archive)>2:
            ...
            efam_div = po.calculate_diversity(overlapping_fitness,
                                          normalized=True)
