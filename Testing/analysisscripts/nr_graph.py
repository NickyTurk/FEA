...
field_files= [path+"/utilities/saved_fields/sec35mid.pickle"]
...
for fieldfile, field_name in zip(field_files, field_names):
    ...
    for method in methods:
        ...
        if method != 'COMBINED':
            ...
            mf = MultiFileReader(to_find, dir='D:\\Prescriptions\\RF_optimized\\')
...
print(list(all_data['sec35mid']['CCNSGA2']))
ccea_yield = list(all_data['sec35mid']['CCNSGA2'].values())
nsga_yield = list(all_data['sec35mid']['SNSGA2'].values())
fea_yield = list(all_data['sec35mid']['FNSGA2'].values())
comb_yield = list(all_data['sec35mid']['COMBINED'].values())
...
plt.legend(handles=[nsga, ccea, fea, comb],loc='best') #bbox_to_anchor=(0, 1.15, 1., .105),
