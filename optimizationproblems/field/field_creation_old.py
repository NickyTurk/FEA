...
class Field:
    ...
    def create_field(self):
        ...
        (
            self.expected_nitrogen_strat,
            self.max_strat,
            self.min_strat,
        ) = self.calc_expected_bin_strat()
    ...
    def create_strip_trial(self):
        ...
        for i, cell in enumerate(cell_list):
            if x_coord == 0:
                ...
                if self.field_shape.contains(cell.small_bounds) and x_coord == cell.bottomleft_x:
                    # Append cell to current strip if it does
                    grid_cell_list.append(cell)
                elif (
                    not self.field_shape.contains(cell.small_bounds)
        ...
        if file_extension == ".shp":
            for i, point in enumerate(list(aa_points)):
                if point["geometry"]["type"] == "Point":
                    ...
                    i == 0
                    and point["geometry"]["type"] == "Polygon"
                    and len(point["geometry"]["coordinates"][0]) > 1
                ):
                    print("field data line")
                elif (
                    i >= 0
                    and point["geometry"]["type"] == "Polygon"
                    and point["properties"].contains("LONGITUDE")
                ):
    ...
    def define_binning_bounds(self, data_to_bin_on="cells"):
        ...
        if data_to_bin_on == "cells":
            ...
            if self.binning_strategy == "yld_pro":
                self.yield_bounds = ps.esda.mapclassify.Equal_Interval(
                    np.asarray(self.yield_points.datapoints), k=self.num_yield_bins
                ).bins.tolist()
                self.yield_bounds[-1] = self.yield_bounds[-1] + 0.1
                if isinstance(self.protein_points.datapoints, pandas.DataFrame):
                    self.protein_bounds = ps.esda.mapclassify.Equal_Interval(
                        np.asarray(self.protein_points.datapoints), k=self.num_pro_bins
                    ).bins.tolist()
                    self.protein_bounds[-1] = self.protein_bounds[-1] + 0.1
            elif self.binning_strategy == "distr":
                self.yield_bounds = ps.esda.mapclassify.Quantiles(
                    np.asarray(self.yield_points.datapoints), k=self.num_yield_bins
                ).bins.tolist()
                self.yield_bounds[-1] = self.yield_bounds[-1] + 1
                if isinstance(self.protein_points.datapoints, pandas.DataFrame):
                    self.protein_bounds = ps.esda.mapclassify.Quantiles(
                        np.asarray(self.protein_points.datapoints), k=self.num_pro_bins
                    ).bins.tolist()
                    self.protein_bounds[-1] = self.protein_bounds[-1] + 0.1
    ...
    def assign_random_nitrogen_binned(self, cells_to_assign):
        ...
        for cell in cells_to_assign:
            cell.nitrogen = self.nitrogen_list[
                i % len(self.nitrogen_list)
            ]  # random.randint(0, len(self.nitrogen_list)-1)
            i = i + 1
    ...
    def calc_expected_bin_strat(self):
        ...
        for i in range(1, self.num_yield_bins + 1):
            for j in range(1, self.num_pro_bins + 1):
                cells_in_bin = sum(
                    cell.yield_bin == i and cell.pro_bin == j for cell in self.cell_list
                )
