...
class FieldMap:
    ...
    def create_folium_map(self):
        ...
        field_map = folium.Map(
            [
                (reference_cell_top.upperright_y + reference_cell_bottom.bottomleft_y) / 2,
        ...
        folium.Choropleth(
            geo_data=self.field.field_shape,
            data=None,
            columns=["geometry"],
            line_opacity=1,
            fill_opacity=0.4,
        ...
        for cell in self.cell_list:
            ...
            if cell.pro_ != -1 and cell.yield_ != -1:
                popup_string = (
                    '<div class="nitrogen-cell value-%d" id="%d">Previous year protein: %.2f <br> Previous year yield: %.2f <br> Fertilizer prescribed: %d <br> Plot index: %d</div>'
                    % (
                        cell.nitrogen,
                        cell.sorted_index,
                        cell.pro_,
                ...
                popup_string = (
                    '<div class="nitrogen-cell value-%d" id="%d">Previous year yield: %.2f <br> Fertilizer prescribed: %d <br> Plot index: %d</div>'
                    % (
                        cell.nitrogen,
                        cell.sorted_index,
                        cell.yield_,
                ...
                popup_string = (
                    '<div class="nitrogen-cell value-%d" id="%d">Fertilizer prescribed: %d <br> Plot index: %d</div>'
                    % (cell.nitrogen, cell.sorted_index, cell.nitrogen, cell.sorted_index)
                )
            folium.vector_layers.Rectangle(
                bounds=cell.folium_bounds,
                popup=folium.Popup(html=popup_string, parse_html=False),
                color="black",
                weight=0.1,
        ...
        folium.map.Marker(
            ...
            [
                self.field.field_shape.bounds[1] + (1000 / 364567.2),
                self.field.field_shape.bounds[2] + (100 / 364567.2),
            ],
        ...
        field_map.save(
            path_
        )  # Saves .html page to templates dir str(parent_dir) + '//app//templates//prescription_map_2.html'
