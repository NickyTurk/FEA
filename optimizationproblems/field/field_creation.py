...
class Field:
    ...
    def __init__(self, id=0, field_dict=None):
        # initialize values
        self.id = id
        self.conversion_measure = (
            6076.1154855643 * 60
        )  # convert feet to Nautical degrees by dividing by this number
        self.field_name = ""
        ...
        if field_dict:
            ...
            self.yld_file = field_dict[
                "yld_file"
            ]  # file containing yield data points, used to discretize into bins for stratification of nitrogen rates
            self.pro_file = field_dict[
                "pro_file"
            ]  # file containing protein data points of wheat, also used for stratifacation purposes
            self.ab_line_file = field_dict[
                "abline"
            ]  # ab-line file, line connecting starting point A of the farming machine to the end of the first "column" at point B.
            # determines orientation of the field based on this ab-line, i.e., north to south or east to west, or at an angle.
            self.grid_file = field_dict[
                "grid_file"
            ]  # If there is an existing grid, read in the existing file
            self.as_applied_file = field_dict[
                "applied_file"
            ]  # Actual application of fertilizer/seeding rate
            ...
            self.buffer_ = -float(
                field_dict["buffer"]
            )  # How many feet from the boundary should the prescription grid start
            self.strip_trial = field_dict[
                "strip_bool"
            ]  # ability to set up a strip trial instead of a grid trial.
            ...
            self.fertilizer_list_1 = [
                float(re.sub("[^\d\.]", "", x))
                for x in str(field_dict["fertilizer_values_1"])
                .strip("[]")
                .replace(" ", "")
                .split(",")
            ]
            ...
            if field_dict["fertilizer_values_2"] != "[]":
                self.fertilizer_list_2 = [
                    float(re.sub("[^\d\.]", "", x))
                    for x in str(field_dict["fertilizer_values_2"])
                    .strip("[]")
                    .replace(" ", "")
                    .split(",")
                ]
            ...
            self.run_ga = field_dict[
                "run_ga"
            ]  # not really used any more with the latin square trials taking over
            ...
            if self.cell_length_min == self.cell_length_max:
                ...
                if self.angle != 0:
                    rotated_field = rotate(
                        self.field_shape_buffered, -self.angle, origin=self.rotation_center
                    )
    ...
    def calculate_rotation_angle(self):
        ...
        if abline_crs.lower() != self.latlong_crs:
            project = pyproj.Transformer.from_crs(
                pyproj.CRS(abline_crs), pyproj.CRS(self.latlong_crs), always_xy=True
            ).transform
    ...
    def create_field_shape(self):
        ...
        if self.field_crs.lower() != self.latlong_crs:
            ...
            project = pyproj.Transformer.from_crs(
                pyproj.CRS(self.field_crs), pyproj.CRS(self.latlong_crs), always_xy=True
            ).transform
            self.field_shape = transform(project, self.field_shape)
        self.field_shape_buffered = self.field_shape.buffer(
            self.buffer_ / self.conversion_measure, cap_style=3
        )
        if self.angle != 0:
    ...
    def select_plot_length(self, col, min, max):
        ...
        if min != middle:
            ...
            for i in np.arange(number_of_cells):
                ...
                poly = Polygon(
                    [(xmax, ymin_cell), (xmin, ymin_cell), (xmin, ymax_cell), (xmax, ymax_cell)]
                )  # 5
    ...
    def recursive_split_poly(self, poly_to_split, ymin_fld, ymax_fld, init=True):
        ...
        if np.floor(total_width * self.conversion_measure) <= (self.cell_width):
            ...
            if (
                poly_to_split.area * self.conversion_measure * self.conversion_measure
                < self.smallest_cell_area
            ):
                return []
            else:
                print(poly_to_split)
    ...
    def create_variable_length_grid(self):
        ...
        for i, col in enumerate(self.columns):
            area, cell_length, cells = self.select_plot_length(
                col, self.cell_length_min - 1, self.cell_length_max + 1
            )
    ...
    def create_basic_grid_for_field(self):
        ...
        grid_cell_list = []
        if self.angle != 0:
            xmin, ymin, xmax, ymax = rotate(
                self.field_shape, self.angle, origin=self.rotation_center
            ).bounds
            rotated = Polygon(
        ...
        for i in np.arange(cols):
            ...
            for j in np.arange(rows):
                new_grid_cell = GridCell(
                    [ring_xleft_origin, ring_ybottom, ring_xright_origin, ring_ytop]
                )
                polygon_list.append(new_grid_cell.true_bounds)
                if self.angle != 0:
                    grid_cell_list.append(new_grid_cell)
                elif self.angle == 0 and self.field_shape_buffered.contains(
                    new_grid_cell.true_bounds
                ):
                    if first_column and first_row:
                        ab_line_top_point = Point(
                            (new_grid_cell.bottomleft_x + new_grid_cell.upperright_x) / 2,
                            new_grid_cell.upperright_y,
                        )
                        first_row = False
                    ...
                    new_grid_cell.folium_bounds = [
                        [new_grid_cell.bottomleft_y, new_grid_cell.bottomleft_x],
                        [new_grid_cell.upperright_y, new_grid_cell.upperright_x],
                    ]
                    if j - 1 == rows or not self.field_shape_buffered.contains(
                        Polygon(
                            [
                        ...
                        if first_column:
                            ab_line_bottom_point = Point(
                                (new_grid_cell.bottomleft_x + new_grid_cell.upperright_x) / 2,
                                new_grid_cell.bottomleft_y,
                            )
                            first_column = False
        ...
        for rot, cell in zip(rotated, grid_cell_list):
            ...
            if og:
                if self.field_shape_buffered.contains(cell.true_bounds):
                    ...
                    if first_row:
                        ab_line_top_point = Point(
                            (cell.bottomleft_x + cell.upperright_x) / 2, cell.upperright_y
                        )
                        first_row = False
            else:
                if first_row:
            ...
            if count != len(grid_cell_list) - 1:
                ...
                if not self.field_shape_buffered.contains(next_cell.true_bounds):
                    ...
                    if first_column:
                        ab_line_bottom_point = Point(
                            (cell.bottomleft_x + cell.upperright_x) / 2, cell.bottomleft_y
                        )
                        first_column = False
    ...
    def create_strip_trial(self):
            ...
            if x_coord == 0:
                ...
                if x_coord == bottomleft_x:
                    ...
                    if self.field_shape.contains(cell.true_bounds):
                        ...
                        cell_indeces.append(cell.original_index)
                    if (
                        bottomleft_x == next_cell.bottomleft_x
                        and cell.bottomleft_y != next_cell.upperright_y
                    ):
        ...
        for i, s in enumerate(full_strip_set):
            ...
            cell_strip.folium_bounds = [
                [cell_strip.bottomleft_y, cell_strip.bottomleft_x],
                [cell_strip.upperright_y, cell_strip.upperright_x],
            ]
    ...
    def order_cells(self, progressbar=None):
        ...
        if file_extension == ".shp":
            for i, point in enumerate(list(fiona.open(self.as_applied_file))):
                if point["geometry"]["type"] == "Polygon":
                    as_applied_points.append(
                        [
                            Point(
                                float(point["properties"]["LONGITUDE"]),
                                float(point["properties"]["LATITUDE"]),
                            ),
                            i,
                        ]
                    )
        ...
        if file_extension == ".csv":
            ...
            for i, row in data.iterrows():
                if is_hex(row["geometry"]):
                    wkt_point = wkb.loads(
                        row["geometry"], hex=True
                    )  # ogr.CreateGeometryFromWkb(bts)
                else:
        ...
        if latlong:
            project = pyproj.Transformer.from_crs(
                pyproj.CRS(self.aa_crs), pyproj.CRS(self.latlong_crs), always_xy=True
            ).transform
        ...
        for p, point in enumerate(good_as_applied_points):
            while i < len(ordered_cells):
                cell = next(
                    x for x in self.cell_list if x.original_index == ordered_cells[i].original_index
                )
                if latlong:
                ...
                if bounds.contains(point) and cell.is_sorted:
                    ...
                    if progressbar is not None:
                        progressbar.update_progress_bar(
                            "Ordering cell... " + str(og_index),
                            (counter / len(self.cell_list)) * 100,
                        )
    ...
    def define_binning_bounds(self, data_to_bin_on="cells"):
        ...
        if data_to_bin_on == "cells":
            ...
            if self.binning_strategy == "yld_pro":
                ...
                # --> 7, 11, 15
                self.yield_bounds = [
                    yield_values[0] + (step_size * (i + 1)) for i in range(self.num_yield_bins)
                ]
                self.yield_bounds[-1] = (
                    self.yield_bounds[-1] + 1
                )  # Because 15 needs to be included, final bound + 1
                # Repeat for protein
                if self.cell_list[0].pro_ != 0:
                    pro_steps = (pro_values[-1] - pro_values[0]) / self.num_pro_bins
                    self.protein_bounds = [
                        pro_values[0] + (pro_steps * (i + 1)) for i in range(self.num_pro_bins)
                    ]
            ...
            # E.g.: There are 9 cells with ordered yields [3,4,4,9,9,9,12,14,15]
                ...
                self.yield_bounds = [
                    yield_values[round((step_size * (i + 1))) - 1]
                    for i in range(self.num_yield_bins)
                ]
                ...
                if self.cell_list[0].pro_ != 0:
                    ...
                    self.protein_bounds = [
                        pro_values[round((pro_steps * (i + 1))) - 1]
                        for i in range(self.num_pro_bins)
                    ]
                    self.protein_bounds[-1] = self.protein_bounds[-1] + 1
    ...
    def assign_random_binned_applicator(self, cells_to_assign, to_apply):
        ...
        for cell in cells_to_assign:
            cell.applicator_value = to_apply[
                i % len(to_apply)
            ]  # random.randint(0, len(self.fertilizer_list_1)-1)
            i = i + 1
...
class Column:
    ...
    def adjust_polygon(self):
        ...
        new_polygon = Polygon(
            [
                (self.xmin, ymins[1] + tens),
                (self.xmax, ymins[1] + tens),
                (self.xmax, ymaxs[0] - tens),
                (self.xmin, ymaxs[0] - tens),
            ]
        )
...
class GridCell:
    ...
    def __init__(self, coordinates):
        ...
        self.gridcell_size = (
            self.true_bounds.area * self.conversion_measure * self.conversion_measure
        )
...
class DataPoint:
    ...
    def set_datapoints(self, gridcell):
        points_in_cell = self.datapoints[
            (self.datapoints[:, 0] >= gridcell.bottomleft_x)
