...
class OptimizationAnalysis():
    ...
    def __init__(self, dim= 20, functions = ['F3', 'F5', 'F11', 'F17', 'F19']):
        self.dim = dim
...
class FactorAnalysis():
    ...
    def boxplot_sizes(self, sizes, title):
        ...
        data_b = sizes[0]
        bpr = plt.boxplot(data_b, positions=np.array(range(len(data_b))) * len(sizes), sym='', widths=1)
        ...
        plt.xlim(-1, len(ticks) * len(sizes))
        plt.tight_layout()
        plt.show()
    ...
    def run_factor_stats_per_function(self, filename_extension, dimension=50, epsilon=0.001):
        group_avgs = []
        group_sizes = []
        overlap_sizes = []
    ...
    def run_factor_stats_one_function(self, filename_extension, function, dimension=50, epsilon=0.001):
...
if __name__ == '__main__':
    ...
    # fctAnl.run_factor_stats_per_function("_overlapping_diff_grouping_small_epsilon.csv", dimension=50)
    # fctAnl.run_factor_stats_per_function("_spectral.csv", dimension=50, epsilon=0)
    # fctAnl.run_factor_stats_per_function("_m4_diff_grouping_small_epsilon.csv", dimension=50)
    # fctAnl.boxplot_sizes(fctAnl.overlap_sizes_per_method, 'ODG Overlap Size Statistics for 50 Dimensions')
